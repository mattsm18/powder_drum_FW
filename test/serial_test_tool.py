...
class SerialTestTool(tk.Tk):
    ...
    def _build_protocol_bar(self, initial_protocol: Path) -> None:
        frame = ttk.LabelFrame(self, text="Protocol", padding=(8, 6))
        frame.pack(fill="x", padx=10, pady=(10, 4))

        self.protocol_path_var = tk.StringVar(value=str(initial_protocol))
        ttk.Entry(frame, textvariable=self.protocol_path_var).pack(
            side="left", fill="x", expand=True, padx=(0, 6)
        )
        ttk.Button(frame, text="Browse", command=self._browse_protocol).pack(side="left", padx=(0, 4))
        ttk.Button(frame, text="Reload", command=self._load_protocol).pack(side="left")

        self.protocol_status_var = tk.StringVar(value="Not loaded")
        ttk.Label(frame, textvariable=self.protocol_status_var).pack(side="left", padx=(12, 0))
    ...
    def _build_connection_bar(self) -> None:
        frame = ttk.LabelFrame(self, text="Connection", padding=(8, 6))
        ...
        ttk.Button(frame, text="Refresh", command=self._refresh_ports).pack(side="left", padx=(0, 12))

        ttk.Label(frame, text="Baud").pack(side="left")
        self.baud_var = tk.StringVar(value="115200")
        ttk.Entry(frame, textvariable=self.baud_var, width=8).pack(side="left", padx=(4, 12))
        ...
        self.conn_status_lbl = ttk.Label(frame, textvariable=self.conn_status_var, foreground="#b00020")
        self.conn_status_lbl.pack(side="left", padx=(12, 0))
    ...
    def _build_main_panes(self) -> None:
        ...
        param_frame = ttk.LabelFrame(panes, text="Parameters", padding=6)
        panes.add(param_frame, weight=3)

        columns = ("id", "name", "access", "type", "unit", "group")
        self.param_tree = ttk.Treeview(param_frame, columns=columns, show="headings", height=12)
        for col, width in zip(columns, (52, 150, 52, 52, 64, 80)):
        ...
        actions = ttk.Frame(param_frame)
        actions.pack(fill="x", pady=(8, 0))
        ttk.Label(actions, text="Value").pack(side="left")
        self.param_value_var = tk.StringVar(value="0.0")
        ttk.Entry(actions, textvariable=self.param_value_var, width=12).pack(side="left", padx=(4, 8))
        ttk.Button(actions, text="GET", command=self._send_get).pack(side="left", padx=(0, 4))
        ttk.Button(actions, text="SET", command=self._send_set).pack(side="left")

        raw_frame = ttk.LabelFrame(panes, text="Raw packet", padding=6)
        panes.add(raw_frame, weight=2)

        for row, (label, attr, width) in enumerate(
            (("Message", "raw_msgid_var", 16), ("Direction", "raw_dir_var", 16))
        ):
            ttk.Label(raw_frame, text=label).grid(row=row, column=0, sticky="w", pady=2)
            var = tk.StringVar()
            setattr(self, attr, var)
            combo = ttk.Combobox(raw_frame, textvariable=var, width=width, state="readonly")
            combo.grid(row=row, column=1, sticky="ew", pady=2)
            if attr == "raw_msgid_var":
                self.raw_msgid_combo = combo
            else:
                self.raw_dir_combo = combo

        ttk.Label(raw_frame, text="Payload (hex)").grid(row=2, column=0, sticky="nw", pady=(6, 2))
        self.raw_payload_var = tk.StringVar(value="20 00 00 80 3F")
        ttk.Entry(raw_frame, textvariable=self.raw_payload_var).grid(
            row=2, column=1, sticky="ew", pady=(6, 2)
        )

        ttk.Label(raw_frame, text="Version").grid(row=3, column=0, sticky="w", pady=2)
        self.raw_version_var = tk.StringVar()
        ttk.Entry(raw_frame, textvariable=self.raw_version_var, width=8).grid(
            row=3, column=1, sticky="w", pady=2
        )

        ttk.Label(raw_frame, text="Length").grid(row=4, column=0, sticky="w", pady=2)
        self.raw_length_var = tk.StringVar()
        ttk.Entry(raw_frame, textvariable=self.raw_length_var, width=8).grid(
            row=4, column=1, sticky="w", pady=2
        )

        self.corrupt_crc_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(raw_frame, text="Corrupt CRC", variable=self.corrupt_crc_var).grid(
            row=5, column=0, columnspan=2, sticky="w", pady=(6, 2)
        )

        ttk.Button(raw_frame, text="Send", command=self._send_raw).grid(
    ...
    def _build_test_bar(self) -> None:
        frame = ttk.Frame(self, padding=(10, 0))
        frame.pack(fill="x")

        ttk.Button(frame, text="Run edge-case suite", command=self._run_edge_case_suite).pack(side="left")
        self.suite_status_var = tk.StringVar()
        ttk.Label(frame, textvariable=self.suite_status_var).pack(side="left", padx=(12, 0))
